...
import { getContestByIdThunk } from "@/entities/contests/model/getTestContestByIdThunk";

const HackathonDetailPage: React.FC = () => {
  const params = useParams();
  const id = params.id as string;
  const dispatch = useAppDispatch();
  const { event } = useAppSelector((state) => state.contest);

  useEffect(() => {
    if (id) {
      dispatch(getContestByIdThunk(id));
    }
  }, [dispatch, id]);
