...
export const Button = (props: ButtonProps | LinkButtonProps) => {
  ...
  const variants: Record<ButtonVariant, string> = {
    primary: 'bg-sky-700 hover:bg-sky-600 shadow-2xl shadow-sky-700/50 text-white',
  ...
  const classes = `${baseStyles} ${variants[variant]} ${sizes[size]} ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`.trim();
