...
import {
  setActiveKey,
  toggleGroup,
} from '@/features/navigate-menu/model/navigate.slice';
...
const NAV_ITEMS: MenuItem[] = [
  ...
  },
  {
    ...
    subItems: [
      {
        key: ENavigationKey.HackathonFind,
        label: 'Найти',
        path: '/hackaton/find',
      },
      { key: ENavigationKey.HackathonMy, label: 'Мои', path: '/hackaton/my' },
      {
        key: ENavigationKey.HackathonCreate,
        label: 'Создать',
    ...
    <nav className='flex flex-1 flex-col gap-1 overflow-y-auto px-4 py-4'>
      {NAV_ITEMS.map((item) => {
        ...
        const buttonClass = isActive
          ? 'w-full flex items-center gap-3 p-3 rounded-xl bg-sky-600/20 text-white'
          : 'w-full flex items-center gap-3 p-3 rounded-xl text-slate-400 hover:bg-slate-800/40';
        const chevronClass = isGroupOpen
          ? 'transition-transform rotate-180 text-sky-400'
